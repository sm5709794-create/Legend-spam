    ...
    def ToKen_GeneRaTe(self, U, P):
        try:
            if U and P:
                ...
                self.Version, self.V = "2019118695", "1.120.2"
                self.PyL = {
...
def load_accounts(file_path="SPIDY.json"):
...
def StarT_SerVer():
    os.system("clear")
    print(render("SPIDY", colors=["blue", "yellow"], align="center"))
    TexT = f"bot activate ꔪ\n\nyt : SPIDY LEGACY"
    panel = Panel(
        Align.center(TexT),
        title="[bold yellow]v - badge[/bold yellow]",
        border_style="bright_blue",
        padding=(1, 2),
        expand=False,
    )
    console.print(panel)
# Added by SPIDY LEGACY GAMER  
    # Ask the user to provide a target group ID (optional)
    forced_target_input = input("type target uid ꔪ ").strip()
